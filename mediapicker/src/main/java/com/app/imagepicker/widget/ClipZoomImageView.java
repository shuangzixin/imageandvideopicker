...
public class ClipZoomImageView extends AppCompatImageView
            ...
            new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
                ...
                @Override public boolean onDoubleTap(MotionEvent e) {
                    ...
                    if (getScale() < SCALE_MID) {
                        ClipZoomImageView.this.postDelayed(new AutoScaleRunnable(SCALE_MID, x, y),
                            16);
                        isAutoScale = true;
                    } else {
                        ClipZoomImageView.this.postDelayed(new AutoScaleRunnable(initScale, x, y),
                            16);
                        isAutoScale = true;
                    }
    ...
    private class AutoScaleRunnable implements Runnable {
        ...
        @Override public void run() {
            ...
            if (((tmpScale > 1f) && (currentScale < targetScale)) || ((tmpScale < 1f) && (
                targetScale
                    < currentScale))) {
    ...
    public Bitmap clip() {
        ...
        draw(canvas);
        return Bitmap.createBitmap(bitmap, getVHorizontalPadding(), getHVerticalPadding(),
            getWidth() - 2 * horizontalPadding, getHeight() - 2 * verticalPadding);
    }
    ...
    private int getVHorizontalPadding() {
    ...
    private int getFrameSize() {
        double bitmapRadio = bitmapWidth * 1.0 / bitmapHeight;
        double cropRadio = cropWidth * 1.0 / cropHeight;
        double Radio = getWidth() * 1.0 / getHeight();
        if(bitmapWidth>cropWidth){
            if(bitmapHeight>cropHeight){
                if(bitmapRadio>cropRadio){
                    return (int) (cropHeight*bitmapRadio);
                }else{
                    return cropWidth;
                }
            }else{
        ...
        //if (bitmapRadio > cropRadio) {
        //    return (int) (bitmapRadio * cropHeight);
        //} else {
        //    return (int) (cropWidth / bitmapRadio);
        //}
    }
